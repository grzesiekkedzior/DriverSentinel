...
RichHeaderController::RichHeaderController(QTableView *mainTableView,
                                           Ui::MainWindow *ui,
                                           QObject *parent)
    : QObject(parent)
    , m_mainTableView(mainTableView)
    , m_ui(ui)
    , m_richHdrModel{QSharedPointer<RichHdrModel>::create()}
    , m_richHeaderData{QSharedPointer<RichHeaderData>::create()}
{
    m_ui->richHdrTableView->resizeColumnsToContents();
    connect(m_mainTableView,
            &QTableView::clicked,
            this,
            &RichHeaderController::loadRichHeaderDataToView);
}

QSharedPointer<RichHdrModel> RichHeaderController::richHdrModel() const
{
    return m_richHdrModel;
}

void RichHeaderController::loadRichHeaderDataToView(const QModelIndex &index)
{
    if (!index.isValid())
        return;
...
void RichHeaderController::clear()
{
    if (!m_richHdrModel)
        return;
    m_richHdrModel->setRichHdrSection({});
}

void RichHeaderController::updateModel(const QVector<RichHeaderData> &richHdrData)
{
    if (!m_richHdrModel)
        return;
    m_richHdrModel->setRichHdrSection(richHdrData);
}

// thank you richier and hasherezade
QString RichHeaderController::getCompilerVersion(quint16 productId)
{
